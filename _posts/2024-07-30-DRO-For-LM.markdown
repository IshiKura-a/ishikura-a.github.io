...
$$
\hat{\theta}\_{\text{DRO}} := \arg\min\_{\theta \in \Theta} \\{ \hat{\mathcal{R}}(\theta) := \max\_{g \in \mathcal{G}} \mathbb{E}\_{(x,y) \sim \hat{P}\_g} [\ell(\theta; (x,y))] \\}.
$$
...
$$
Generally, $\log{p_{x|z}(x|z)}$ is hard to estimate, in practice, we can train a baseline model (like bigram model) for each task.
...
![Doremi Framework](/assets/240807000.png)
